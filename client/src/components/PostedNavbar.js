      ...
      if (userInfo.team_set && userInfo.team_set.length === 0) {
        ...
        for (let i = 0; i < userInfo.team_set.length; i++) {
          axios
            ...
            .then(
              (res) =>
                (teamsInfo[i] = { name: res.data.team_name, id: res.data.id })
            );
        }
    ...
    <Navbar bg="light" expand="lg">
      <Container>
        <a href="/" style={{ textDecoration: "none", color: "inherit" }}>
          <h1 className="nav-title">Post-ed</h1>
        </a>
              ...
              {teamsInfo !== "None" &&
                teamsInfo.map((value, index) => {
                  return (
                    <NavDropdown.Item
                      key={index}
                      onClick={() => handleGroupClick(value)}
                    >
                      {value.name}
                    </NavDropdown.Item>
                  );
            ...
            <Nav>
              ...
              >
                <FormControl
                  ...
                  aria-label="Search"
                />
                <Button variant="outline-success" type="sumbit">
                  Search
                </Button>
              </Form>
            </Nav>
